    ...
    public class LeaveDockDateCrawler
    {
        public static string FindExportDate(string Conveyance, string VoyageNumber, string strDock)
        {
            ...
            switch (strDock)
            {
                ...
                case "外二期":
                    strRet =
                            GetDataFromSite("http://211.144.118.73/search/comm/shipname.jsp?ename=", Conveyance,
                                        VoyageNumber,
                                        new DataFormat("<tr class=\"result_tr", "</tr>", "<td>", "</td>", 0, 2, 6));
                    break;
                case "外四期":
                ...
                case "外五期":
                    strRet =
                            GetDataFromSite("http://www.smct.com.cn:81/search/comm/shipname.jsp?ename=", Conveyance,
                                        VoyageNumber,
                                        new DataFormat("<tr class=\"result", "</tr>", "<td align=\"center\" class=\"x2\">", "</td>", 0, 2, 6));
                    break;
                default:
                    strRet =
                        GetDataFromSite("http://www.sctport.com.cn/newsctsite/sct/detailves.jsp?hename=", Conveyance,
                                        VoyageNumber,
                                        new DataFormat("<tr onMouseOver=\"this.style.backgroundColor='#CCCCCC'\"",
                                                       "</tr>", "<td><div align=\"center\"><font size=\"2\">", "</font></div></td>", 0, 2, 5));
                    break;
            }
        ...
        private static string ParsePortData(string htmlContent, string Conveyance, string VoyageNumber, DataFormat dataFormat)
        {
            ...
            {
                //add detail
                List<string> detailList = HtmlParseUtils.GetSubStrings(content, dataFormat.TRHead, null, dataFormat.TRTail, null, null, null);
                if (detailList != null)
                {
                    foreach (string s in detailList)
                    {
                        List<string> list =
                            HtmlParseUtils.GetSubStrings(s, dataFormat.TDHead, null, dataFormat.TDTail,
                                                         dataFormat.TDHead, dataFormat.TDTail, null);
                        if (list != null)
                        {
                            try
                            {
                                ...
                                {
                                    if (list[dataFormat.VoyageNumberColumnNumber].Contains(VoyageNumber))
                                        LeaveDateList.Add(list[dataFormat.LeaveDateColumnNumber]);
                                }
                            }
                            catch (Exception ex)
